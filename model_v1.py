...
from AutoCV.kakaobrain.model import Model as AutoCVimageModel
from AutoCV.video.model import Model as AutoCVvideoModel
DOMAIN_TO_MODEL = {
                   'text': AutoNLPModel,
...
class Model():
    ...
    if is_training:
      ...
      if(self.loop_num<1):
        ...
        if self.start:
          ...
          # dataset = dataset.map(f, num_parallel_calls = 10)
        ...
        while True:
          try:
            ...
            # self.total_num = self.domain_metadata['train_num']
            if(len(label_set)==self.domain_metadata['class_num']) and len(self.Y) > self.total_num * 0.3:
                break
        ...
        while True:
          try:
            ...
            self.X.append(example)
            self.Y.append(labels)
            if len(self.Y) > self.total_num * 0.8:
              break
          except tf.errors.OutOfRangeError:
            break
        setattr(self, attr_X, self.X)
      ...
      if not (hasattr(self, attr_X) and hasattr(self, attr_Y)):
        # dataset = dataset.padded_batch(64, padded_shapes=([-1, 1, 1, 1], [-1]))
        dataset = dataset.prefetch(50)
        iterator = dataset.make_one_shot_iterator()
  ...
  def to_numpy_speech(self, dataset, is_training):
    ...
    if is_training:
      ...
      if (self.loop_num < 1):
        ...
        while True:
          try:
            ...
            if (len(label_set) == self.domain_metadata['class_num']):
             self.num = i
             if (self.domain_metadata['train_num'] <= 300):
               if (i == max(self.num, 50)):
                 break
             elif (self.domain_metadata['train_num'] >= 1000):
               if (i == max(self.num, 500)):
                 break
             else:
               if (i == max(self.num, 200)):
                 break
            # if(len(label_set) == self.domain_metadata['class_num']):
            #   break
            print(len(label_set))
      ...
      # Only iterate the TF dataset when it's not done yet
      if not (hasattr(self, attr_X) and hasattr(self, attr_Y)):
        # dataset = dataset.padded_batch(2, padded_shapes=([-1, 1, 1, 1], [-1]))
        dataset = dataset.prefetch(50)
        iterator = dataset.make_one_shot_iterator()
        next_element = iterator.get_next()
        X = []
  ...
  def to_numpy(self, dataset, is_training):
    ...
    if is_training:
      ...
      if not (hasattr(self, attr_X) and hasattr(self, attr_Y)):
        ...
        dataset = dataset.batch(sample_count)
        iterator = dataset.make_one_shot_iterator()
        next_element = iterator.get_next()
        X = []
        Y = []
    ...
    if is_training:
      ...
      if self.loop_num < cn:
        ...
        if self.domain == 'text':
          ...
          x_columns_num = X.shape[4]
          X = X.reshape(-1, x_columns_num)
