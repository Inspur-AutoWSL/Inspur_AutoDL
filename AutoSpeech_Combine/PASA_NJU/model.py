...
from .model_second import ModelSecond
...
class Model(object):
    ...
    def train(self, train_dataset, remaining_time_budget=None):
        ...
        if(remaining_time_budget>1120):
        # if self.train_loop_num <= self.over_num:
        ...
        if(remaining_time_budget<1120):
            if self.start:
                ...
                #from keras.backend import clear_session
                #K.clear_session()
                self.second_model = ModelSecond(self.metadata)
                self.start = False
            self.second_model.train(train_dataset = train_dataset, remaining_time_budget=remaining_time_budget)
    ...
    def test(self, test_x, remaining_time_budget=None):
        ...
        if(remaining_time_budget>1120):
            ...
            self.last_pred = pred_y
            
        # if self.train_loop_num > self.over_num:  
        if(remaining_time_budget<1080):
            pred_y = self.second_model.test(test_x, remaining_time_budget=remaining_time_budget)
        else:
            pred_y = self.last_pred
