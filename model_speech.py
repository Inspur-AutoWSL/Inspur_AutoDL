...
class Model():
  ...
  def to_numpy(self, dataset, is_training):
    ...
    if not (hasattr(self, attr_X) and hasattr(self, attr_Y)):
      ...
      example = tf.reshape(dataset, [5,-1])
      
      sample_rate = 16000.0

      stfts = tf.signal.stft(example, frame_length=1024, frame_step=256, fft_length=1024)
