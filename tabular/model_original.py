...
class Model(object):
  ...
  def train(self, dataset, remaining_time_budget=None):
    ...
    if(self.START):
        ...
        #X_=[list(x).index(max(x)) for x in y_train]
        #y=[list(x).index(max(x)) for x in y_test]
                                                                    
        #### generation: pipeline的循环数一般越多越好，population_size：每一个循环数中GP中的个体数，一般越多越好
        # tpot = TPOTClassifier(generations=1, population_size=1, verbosity=1)
        # tpot.fit(X_train, X_)
        # print(tpot.score(X_test, y))
        # tpot.export('tpot_digits_pipeline.py')

        #data = pd.DataFrame(X)
        #length = data.shape[1]
        #data.columns = [str(i) for i in range(0,length)]
        #print(data.columns)
        ...
        #feature_matrix, feature_defs = ft.dfs(entityset = es, target_entity = 'data',trans_primitives = ['multiply_numeric'])
        ...
        y_stack = [list(x).index(max(x)) for x in y]
        ...
        basemodel4 = KNeighborsClassifier(n_neighbors=2)
        basemodel5 = AdaBoostClassifier()
        basemodel6 = BaggingClassifier()
        basemodel7 = GradientBoostingClassifier()         
        
        lr = lgb.LGBMClassifier()
        # lr = LogisticRegression()
        self.sclf = StackingClassifier(classifiers=[basemodel2, basemodel7], 
                                  meta_classifier=lr)
         
        print('5-fold cross validation:\n')
         
        for basemodel, label in zip([basemodel2, basemodel7, self.sclf], 
                              ['lightgbm',
                               'gbdt',        
                               'StackingClassifier']):
         
            scores = model_selection.cross_val_score(basemodel,X, y_stack, 
                                                      cv=5, scoring='roc_auc')
            print("Auc: %0.2f (+/- %0.2f) [%s]" 
                  % (scores.mean(), scores.std(), label))
        
        self.sclf.fit(X,y_stack)
        ...
        print("--------------starttrain")
        self.classifier = lgb.train({**params, **self.hyperparams}, train_data, 5, valid_data, init_model=self.classifier, early_stopping_rounds=10, 
                                  verbose_eval=10, keep_training_booster=True)
        print("--------------endtrain")
        train_end = time.time()
        print(train_end - train_start)
